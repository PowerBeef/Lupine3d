...
The v0.8 visual/performance tradeoff was explicitly accepted, and later releases inherit it unchanged; the textured renderer's cost was accepted the same way (`docs/TEXTURED_WALLS.md`). This does not relax safety limits or enable unrelated experimental kernels. Keep failed performance-gate evidence and the original baseline comparisons intact.
