...
def surface_attributes(profiles: list[int], page: int) -> bytes:
    result = bytearray(VIEW_MAP_BYTES)
    for column in range(32):
        ...
        for row in range(VIEW_ROWS):
            result[row * 32 + column] = (page << 3) | (upper if row < FOLDED_ROWS else lower) | (0x40 if FOLDED_COMPOSITOR and row >= FOLDED_ROWS else 0)
    return bytes(result)
...
def emit_surfaces(a: Assembler) -> None:
    ...
    a.ld_rr_nn("de", 32)
    for row in range(VIEW_ROWS):
        a.ld_r_r("a", "b" if row < FOLDED_ROWS else "c"); a.ld_hl_a(); a.add_hl_rr("de")
    ...
    if ATTRIBUTE_PADDING:
        ...
        for row in range(VIEW_ROWS):
            ...
            a.ld_r_r("a", "b" if row < FOLDED_ROWS else "c")
