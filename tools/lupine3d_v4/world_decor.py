...
def emit_world_decor(a: Assembler):
    ...
    if PHYSICAL_DEPTH:
        a.ld_a_abs(COVERAGE_MODE); a.or_r("a"); a.jr("fixture_capacity_ready","nz")
    a.ld_a_abs(DECAL_USED); a.cp_n(4); a.jp("fixture_done","nc")
    if PHYSICAL_DEPTH: a.label("fixture_capacity_ready")
    ...
    a.ld_a_abs(SENTINEL_SCREEN_X); a.add_a_r("b"); a.ld_abs_a(MASK_OAM_X)
    if PHYSICAL_DEPTH:
        a.ld_a_abs(COVERAGE_MODE); a.or_r("a"); a.jr("fixture_actual_mask","z")
        a.ld_a_abs(MASK_OAM_X); a.sub_n(8); a.ld_r_n("b",8); a.call("coverage_mark_strip"); a.jp("fixture_next_column")
        a.label("fixture_actual_mask"); a.ld_a_abs(MASK_OAM_X)
    a.sub_n(8); a.call("fixture_mask")
