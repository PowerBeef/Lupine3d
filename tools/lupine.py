...
tools do; it only spares the reader the flag spelling and the script names.
    ...
    lupine build [--textured] [--display slim|compact|legacy] [--output-dir DIR]
    lupine run   [--scenario FILE] [--textured] [--snapshot-mode check|record|none]
    ...
    lupine profile [--textured]       cycles by main-loop stage on the coherence tour
    lupine test | witnesses | release-check | sable-check [--textured]
    lupine symbols [--textured]       where the debugger exports are and how to load them
"""
...
def output_dir(args) -> Path:
    ...
    return ROOT / "build" / ("textured" if getattr(args, "textured", False) else "")
...
def cmd_witnesses(args) -> int:
    ...
    if args.textured:
        command += ["--output-dir", ROOT / "build" / "independent-witnesses-textured"]
    return run(command, profile_env(args))
...
def cmd_sable_check(args) -> int:
    ...
    if args.textured:
        command += ["--output-dir", ROOT / "build" / "sable-v2" / "textured-checks"]
    return run(command, profile_env(args))
...
def cmd_symbols(args) -> int:
    ...
    if not manifest.is_file():
        print(f"no build under {out}: run `lupine build{' --textured' if args.textured else ''}` first")
        return 1
...
def main(argv: list[str] | None = None) -> int:
    ...
    def profile_options(p):
        p.add_argument("--textured", action="store_true", help="the textured-walls profile (LUPINE3D_TEXTURED_WALLS=1)")
    ...
    p.add_argument("--output-dir", help="default build/, or build/textured with --textured"); p.set_defaults(func=cmd_build)
