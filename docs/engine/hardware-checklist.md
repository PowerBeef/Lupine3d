# Lupine 3D — Independent Emulator and Original Hardware Checklist
...
The owner currently has no physical Game Boy Color or flash cartridge. The
physical sections below are retained for possible future access and do not
block emulator-qualified development or releases. Record hardware status as
unavailable / untested; continue to run the independent emulator checks.

## Test record

| Field | Value |
|---|---|
| ROM version | Copy from `VERSION` |
| ROM SHA-256 | Copy from the matching release's `SHA256SUMS.txt` |
...
Expected page protocol: hidden dynamic BG patterns target $9000 (on slim also $8800, for pattern ids 128 and above), masked OBJ patterns target $8000, attributes target the hidden map in bank 1, and tile IDs target the same map in bank 0. HUD/OAM publish before LCDC flips. On slim the patterns and the hidden map stream by HBlank DMA during composition and the VBlank interrupt publishes the tail (`vblank_tail`) while the main loop casts the next update. The legacy profile stages large packets: hidden dynamic patterns go in an earlier VBlank, the final visible packet remains coherent, and the maximum payload is 176 blocks, split into at most 96 and 80 blocks.
...
The legacy profile's maximum full packet is 176 blocks, with at most 96 blocks in the first stage and 80 in the last, staged across two VBlanks. Compact and slim stream instead: up to 96 (compact) or 238 (textured slim) dynamic-pattern blocks and the 28/30-block hidden map go by HBlank DMA during composition (one block per visible line, into the hidden bank and hidden map only), and the tail is one VBlank of at most 62 GDMA blocks (32 masks + 30 attributes) plus the HUD writes, OAM DMA and flip, run on slim by the VBlank interrupt itself. On hardware, watch for: a transfer started outside an idle `HDMA5`, a VBK change while a transfer is active, an HBlank block reading a banked WRAM source (both streamed sources are fixed WRAM), and any tearing in the displayed page, which would mean a block landed in the visible bank. Cached packets transfer at most 32 blocks in one VBlank. Transfer time alone is insufficient: include HUD writes, OAM DMA/wait and publication bookkeeping, and require the tail's completion before line 153.
