...
Lupine 3D 0.6.2 presents the first gameplay slice as a navigable industrial-horror level with an exact wall-rendering contract. The renderer benchmark and its nine RGB oracles remain isolated from the state-driven Hangar Breach scenario.
...
Machinery panels use world-cell ribs and a half-height rail rather than screen-space texture. The rail is enabled only in the entity-heavy scene profile and only for uninterrupted eight-pixel material spans. The renderer-heavy profile spends those two tile IDs on rare seam masks and retains its lean compositor path.
...
The 0.6.2 software gate requires:

- all 36 unit and ROM-execution tests passing;
