...
| Every actor on a walkable cell the spawn reaches with the enemies-cleared doors shut | those doors open only once every actor is dead, so an actor behind one, or inside a wall, could never be engaged and the route would deadlock |
