...
false. The previous release is retained in [the v0.9 report](archive/TEST_REPORT_V09.md).
...
| Textured profile (opt-in, `LUPINE3D_TEXTURED_WALLS=1`; ROM SHA-256 `ce1fe245fce224392870ca749e1240ee1c9cd50e36c86d6f02db64bd92346bcd`) | The emitted row-window kernel (`docs/TEXTURED_WALLS.md`) passes the coherence (11 updates), living-world (47) and art (6) tours with every frame check, including `dynamic_tiles_exact` against `texture_reference.compose_kernel` read from VRAM, `ray_u_exact`, `pixel_u_exact` and zero mode-3 writes; `tools/check_sable.py` under the flag passes 19 checks including blind composition that laps the 96-slot ring and crosses the VRAM half, publication windows up to 160 patterns and the chunked HBlank hand-off; pinned SameBoy CGB-0/CGB-E and mGBA pass with zero unsafe transfers; 122 goldens under `snapshots/slim-sable-v2-textured/`, four of which (`world` 11-13, `art` 06) were re-accepted on this ROM after the kernel performance round because faster updates land on other accepted snapshot ticks (a weapon cel, the helmet blink, a fixture cel, a Sentinel walk cel), with every frame check unchanged; randomised power-on RAM images boot and validate identically in the harness, and 500 explicit SameBoy seeds pass the frozen-scene lane once the adapters treat world entry as the ROM's own write of the playing mode rather than a read (a pre-existing adapter flaw that timed out about one run in 150 on random RAM, on the v0.10 ROM as well). Measured over every code region of the coherence tour the kernel costs about 270k T per full update against the flat compositor's 60k (tour mean 917k T against 675k; an earlier figure of +140k came from a profile truncated to its top regions), so the profile is not the default: the prototype gate is **not met**, and the lab with these measured constants records the projected rates in `research/results/textured_walls_lab_v2.json`. The default ROM is byte-identical |
