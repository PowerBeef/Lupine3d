...
| Wall textures | `assets/textures/*.png` (a structure and a machinery texture per episode, one shared door plate) | 16×8 (upper half) | 1 | row-window tables in `TEXTURE_WINDOW_BANKS` under the textured profile; a level's palette set picks its texture set |
