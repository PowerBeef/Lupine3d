...
CASES = [
    ...
    ("kind drop", game(set_key(["kinds", 0, "drop"], "ammo")), r"kinds\[0\]\.drop 'ammo' must be one of medkit, stim, .*, none"),
    ("ranged keys together", game(lambda d: d["kinds"][2].pop("windup_ticks")),
     r"kinds\[2\]: a ranged kind gives range, ranged_damage and windup_ticks together"),
    ("range", game(set_key(["kinds", 2, "range"], 9)), r"kinds\[2\]\.range must be a whole number from 2 to 7"),
    ("carry over", game(set_key(["carry_over"], "yes")), r"carry_over must be true or false"),
