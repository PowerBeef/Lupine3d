...
The three distances' figures halve in height (32, 16, 8) in the showcase and
the starter, and the engine switches where neighbouring sizes are equally
...
print(json.dumps(dict(heights=heights, widths=widths, columns=l.SENTINEL_MID_COLUMNS,
                      lod=[l.LOD_NEAR_ENTER, l.LOD_NEAR_HOLD, l.LOD_FAR_HOLD, l.LOD_FAR_ENTER])))
"""
...
class ActorSizeLadder(unittest.TestCase):
    def check_game(self, game):
        data = probe(game)
        self.assertEqual(data['heights'], [32, 16, 8], game)
        self.assertEqual(data['columns'], 1, game)
        # Figures are true at forward 16 * 32 / h (Q4): 16, 32 and 64. The
        # switches sit between them, near below 21 (held to 25), far from 48
        # (held down to 42).
        self.assertEqual(data['lod'], [21, 25, 42, 48], game)
